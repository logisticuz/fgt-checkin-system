...
| `tournament_name`         | `Single line text` | The name of the tournament (from Start.gg).                              | `FGC Trollhättan Weekly #42`      |
...
| `personnummer`                | `Single line text` | The participant's personal ID number (sanitized, digits only). Used for matching against Sverok. **Not stored permanently**. | `199001011234`                             |
...
| `tournament_games_registered` | `Multi-select`     | A list of games the participant is registered for in the tournament. | `["Street Fighter 6", "Tekken 8"]`         |
...
| `is_guest`                    | `Checkbox`         | `[x]` if the player was **not** found on Start.gg. Set automatically.      | `[x]`                                      |
...
| `status`                      | `Single select`    | Overall status for the participant: `Ready`, `Pending`.                  | `Ready`                                    |
