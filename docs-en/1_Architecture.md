...
*   **Responsibility:** This is the hub for the public-facing part of the application.
    ...
    *   **API Endpoints:** Provides a JSON API (e.g., `/api/participant/{name}/status`) that is used by the status pages to poll for updates.
...
*   **Responsibility:** This is the primary tool for the Tournament Organizers (TOs).
    ...
    *   **Real-time Overview:** Displays a live-updated table with all checked-in participants and their status (Green, Red, etc.). It also has a "Needs Attention" section to quickly identify who needs help.

#### 3. N8N (`n8n/`)
*   **Technology:** n8n.io (Workflow Automation)
*   **Responsibility:** This is the "brain" of the system that executes the actual check-in logic.
    ...
    *   **Business Logic:** When a `checkin` request arrives, `n8n` executes a workflow that performs the following steps:
        ...
        5.  Returns a `JSON` response to the `backend` indicating if the check-in was successful or what is missing.
